...
export function MeetingChatPanel({ meetingId, currentUserId, onClose }: MeetingChatPanelProps) {
  ...
  const listRef = useRef<FlatList<MeetingMessage>>(null);

  useEffect(() => {
    let cancelled = false;
    listMeetingMessages(meetingId)
  ...
  const handleIncoming = useCallback((msg: { payload: Uint8Array }) => {
    try {
      const incoming = JSON.parse(decoder.decode(msg.payload)) as MeetingMessage;
  ...
  const handleSend = async () => {
    ...
    try {
      const { message } = await sendMeetingMessage(meetingId, text);
